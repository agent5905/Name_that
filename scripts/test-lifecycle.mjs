...
try {
  ...
  const lobbyPlayerToken=token();const lobbyPlayer=await rpc('join_room',{p_code:roomCode,p_display_name:'Lobby',p_participant_token_hash:hash(lobbyPlayerToken)});
  await rpc('host_action',{p_code:roomCode,p_host_token_hash:hash(hostToken),p_action:'start'});
  const roundZeroOpen=await admin.from('room_snapshots').select('choices').eq('room_code',roomCode).single();
  const scored=await rpc('submit_answer',{p_code:roomCode,p_player_id:lobbyPlayer.playerId,p_participant_token_hash:hash(lobbyPlayerToken),p_employee_id:roundZeroOpen.data.choices[0].id,p_expected_round:0});assert.equal(scored.idempotent,false);
  ...
  await expectMarker('submit_answer',{p_code:roomCode,p_player_id:lockedJoin.playerId,p_participant_token_hash:hash(lockedToken),p_employee_id:snapshot.data.choices[0].id,p_expected_round:0},'ANSWERS_CLOSED');
  ...
  await rpc('host_action',{p_code:roomCode,p_host_token_hash:hash(hostToken),p_action:'show_leaderboard'});
  ...
  for(const action of ['lock','reveal','show_results','show_leaderboard','end'])await rpc('host_action',{p_code:roomCode,p_host_token_hash:hash(hostToken),p_action:action});
  ...
  const againSnapshot=await admin.from('room_snapshots').select('*').eq('room_code',againCode).single();assert.equal(againSnapshot.data.phase,'lobby');assert.equal(againSnapshot.data.connected_participant_count,0);assert.equal(againSnapshot.data.submitted_answer_count,0);assert.equal(againSnapshot.data.round_count,2);assert.equal(againSnapshot.data.leaderboard,null);
  const replayPlayers=await admin.from('players').select('*',{count:'exact',head:true}).eq('room_id',again.roomId);assert.equal(replayPlayers.count,0,'Play Again must not copy scores, streaks, ranks, or player identity.');
  const replayAnswers=await admin.from('session_answers').select('*',{count:'exact',head:true}).eq('room_id',again.roomId);assert.equal(replayAnswers.count,0,'Play Again must start with an empty scoring ledger.');
  ...
  for(const action of ['show_results','next_round','lock','reveal','show_results','show_leaderboard','end'])await rpc('host_action',{p_code:againCode,p_host_token_hash:hash(againToken),p_action:action});
