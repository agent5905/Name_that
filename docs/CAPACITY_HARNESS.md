...
`scripts/load-capacity.mjs` is a protocol-level load generator for the staged 5, 25, 100, 175, and 225 participant scoring checks. Dry-run is the default and performs no network requests. The older 10/50 reports remain historical evidence but are not scoring-renewal stages.
...
The final 225 run additionally uses `--browser-observers`. This launches two independent real Chromium contexts at 1440×900 and 1280×720. The host browser, not the protocol runner, presses every control; the display browser must render every phase and decode the current Mystery/Reveal image. Both contexts contribute one real client/channel to the 227-connection envelope. Console errors, page errors, failed requests, 5xx responses, host/display phase latency, and screenshots become strict report evidence.

## What one simulated client does

Each participant performs its own join, fetches `/api/realtime-auth` once, creates one Supabase client, opens one Realtime WebSocket with one private `room:<code>` channel, hydrates via HTTP, answers each round with the authoritative expected round index, and runs the same 60-second jittered safety reconciliation pattern. No Supabase Anonymous Sign-In is used. Deterministic cohorts produce correct, incorrect, streak, zero-score, duplicate-name, and tie cases. Round 1 shows the optional leaderboard, Round 2 skips it, and the final round shows the required final leaderboard before completion.
...
After each round, the service-role audit path independently reads the isolated room's scoring ledger, choices, question-open timestamp, and player aggregates. It recalculates correctness and the integer `750 + round(250 × (1 - elapsed/20000))` score with the 20-second clamp; validates streak-before/streak-after, cumulative score, correct count, and correct-response time; and checks one answer/award per player/question. At each displayed leaderboard it independently sorts `total score DESC, correct answers DESC, correct response time ASC, player UUID ASC`, compares the exact public Top 5/Top 10, and hydrates every participant to verify personal score/rank, including duplicate display names and deterministic zero-score ties. These privileged reads are harness-only and are not exposed as a production endpoint.
...
PASS requires all intended participants to join and reach Realtime, exactly one channel per simulated browser, the expected join/answer/reconnect operations, no failed snapshots, no malformed or same-phase audience events, no Realtime channel errors, exact transition delivery, exact authoritative result totals, zero score-ledger/aggregate mismatches, exact public leaderboard and personal ranks, successful cleanup, and a healthy load generator. Reports include answer latency and the additional synchronized personalized-hydration latency after Reveal/Leaderboard. Generator thresholds are p95 event-loop delay at most 100 ms, max event-loop delay at most 1,000 ms, and peak RSS at most 1.5 GiB. CPU time and single-core utilization are recorded for review.
