...
returns trigger language plpgsql security definer set search_path='' as $$
declare code text;version bigint;phase text;revealed jsonb;reveal_key jsonb;snapshot jsonb;
begin
  ...
  code:=new.room_code;version:=new.version;phase:=new.phase;revealed:=new.revealed_employee;
  if new.phase in('employee_revealed','results_displayed','complete')and revealed is not null then
    select jsonb_build_object('key',rtrim(translate(encode(q.reveal_key,'base64'),'+/','-_'),'='),
      ...
      where r.code=code and r.content_mode='saved'and q.reveal_key is not null and q.reveal_iv is not null and q.reveal_aad is not null;
  ...
  perform realtime.send(jsonb_build_object('roomCode',code,'version',version,'phase',phase,'snapshot',snapshot),
    'room_snapshot_changed','room:'||code,true);
  return null;
end;$$;
